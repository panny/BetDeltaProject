...
dash_app3.layout=html.Div([ html.Div(html.H1('This is a private application, please leave immediately')),
...
def response_to_lg(nc, lg, ps):
    if nc:
        ...
        xxx= requests.get('https://iosbstxn02.hkjc.com/txnA/AOSBS/Login.ashx?Action=Login_Question&AccOrId='+account['login']+\
               '&Password='+hashlib.sha1(account['pw']).hexdigest()+\
...
#         print(ans, account['auth'])
        ...
        hashlib.sha1(ans.upper().encode()).hexdigest().upper()+\
